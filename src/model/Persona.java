...
//            p.hijos.
    ...
    public String verInfo(Persona p) {
        this.setToViewInfo(this.getToViewInfo() + "\n" + p.getID() + " [margin=0 shape=box, style=filled, fillcolor=white, color=blue, label=<<TABLE border=\"0\" cellborder=\"0\"><TR><TD width=\"60\" height=\"50\" fixedsize=\"true\"><IMG SRC=\"" + p.getFoto() + "\" scale=\"true\"/></TD><td><font point-size=\"10\">" + p.getNombre() + "</font></td></TR></TABLE>>];");
        for (Persona hijo : p.getHijos()) {
            this.verInfo(hijo);
        }
